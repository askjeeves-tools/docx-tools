...
test("homepage has SEO meta tags and structured data", async ({ page }) => {
	...
	await expect(expander.locator("summary")).toHaveText("Supported DOCX conversions");
	await expect(expander.locator("li")).toHaveCount(3);
	expect(await page.content()).toContain("DOCX → HTML");
	...
	await expect(
		seoExpanders.filter({
			has: page.locator("summary", { hasText: "Supported DOCX conversions" }),
		}),
	).toBeVisible();
	await expect(
...
test("static SEO files are served", async ({ request }) => {
	...
	expect(robotsText).toContain(
		"Sitemap: https://docx.askjeeves.cc/sitemap-index.xml",
	);
